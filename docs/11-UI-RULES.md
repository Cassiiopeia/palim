...
**받은 값·담긴 값을 화면에서 다듬지 않는다.** 이 규칙이 필요한 이유는 한 번 겪었기 때문이다 —
품목코드 `00094` 가 화면에서 `94` 로, `01002` 가 `1,002` 로 보였다. 숫자처럼 **생긴** 글자를
숫자로 다듬으면서 앞자리 0 이 사라지고 쉼표가 붙었다. 담긴 값은 멀쩡했다.

**「진짜로 넣기 전에 눈으로 보는」 화면에서는 특히 치명적이다.** 보이는 것과 담긴 것이 다르면
확인 단계 자체가 거짓이 된다. 멀쩡한 자료를 잘못됐다고 보거나, 그 반대로 본다.

읽기 좋게 만드는 일은 **담을 때** 끝낸다. 시각을 사람이 읽는 글자로 담아 두면 화면은 그대로
보여주기만 하면 된다. 화면에서 되돌리려 하면 「어느 칸이 무슨 값인지」를 화면이 또 판단해야
하고, **판단이 두 군데로 갈리면 언젠가 어긋난다.**

허용되는 것은 **값을 바꾸지 않는 처리**뿐이다.

| 해도 되는 것 | 하면 안 되는 것 |
|---|---|
| 중첩 객체를 `…` 로 접기 | 자릿수 구분 기호 붙이기 |
| 너무 긴 글자를 `…` 로 자르기 (원문은 펼쳐 볼 수 있게) | 앞자리 0 지우기 |
| 열 순서·정렬 바꾸기 | 소수점 자리 줄이기 |
| 빈 값을 `—` 로 표시 | 시각·숫자·글자를 화면이 짐작해 변환 |

## 내가 정해도 되는 것과 물어봐야 하는 것

**자의적으로 판단하지 않는다.** 아래 경계를 넘는 것은 코드로 정하지 말고 사장님께 묻는다.

| 내가 정해도 되는 것 | 물어봐야 하는 것 |
|---|---|
| 실패 문구·안내 문장 | **원천 칸의 뜻** — 「이 칸이 품목코드다」 는 값을 본 사람만 안다 |
| 화면 배치·색·간격 | **값을 어떻게 보여줄지** — 위 표 참고 |
| 무엇을 접고 무엇을 펼칠지 | **무엇을 자동으로 채울지** — 그럴듯하게 채우면 확인 없이 넘어간다 |
| 로그에 무엇을 남길지 | **무엇을 버릴지** — 안 쓰는 값도 보관한다 |

**추천을 짐작으로 채우지 않는다.** 사전은 «우리가 아는 이름» 만 잡는다. 뜻이 애매하면 비워 두고
화면이 **실제 값을 보여줘서** 사람이 판단하게 한다. 틀린 추천은 없는 추천보다 나쁘다 —
빈 칸은 채우지만 채워진 칸은 확인하지 않고 넘어간다.
