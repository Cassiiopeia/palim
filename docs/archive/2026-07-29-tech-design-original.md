...
| 알림 전달 | **PostgreSQL Outbox** (메시지 큐 미사용 — 7장 참조) |
| 관리자 화면 | Thymeleaf 서버 렌더링 + **Tailwind CSS 4 / daisyUI 5** + Spring Security |
| 알림 | Telegram Bot API |
| 예외 처리 | `BusinessException` 단일화 + `ErrorCode` enum (4.8 참조) |
...
### 2.4 관리자 화면 — Thymeleaf + daisyUI

화면이 7개이고 사용자가 관리자 1명이다. SPA를 분리하면 인증 처리·빌드 파이프라인·CORS 관리가 추가되지만 얻는 것이 없다. 서버 렌더링으로 단일 배포 단위를 유지한다.

**스타일은 Tailwind CSS 4 + daisyUI 5를 쓰고, daisyUI 컴포넌트를 최대한 그대로 사용한다.**

| 원칙 | 내용 |
|---|---|
| daisyUI 컴포넌트 우선 | `btn`, `card`, `table`, `badge`, `alert`, `modal`, `stat`, `drawer` 등을 그대로 쓴다 |
| 커스텀 CSS 최소화 | 별도 스타일시트를 만들지 않는다. 필요하면 Tailwind 유틸리티로 해결한다 |
| 테마 | daisyUI 내장 테마를 사용한다. 색상을 직접 지정하지 않는다 |
| 반응형 | PC·모바일 모두 사용하므로 Tailwind 브레이크포인트를 쓴다(F-09) |

> **설계 근거**: 관리자 1명이 쓰는 내부 도구에 디자인 시스템을 자체 구축할 이유가 없다. 커스텀 CSS를 쌓으면 유지보수 대상만 늘어난다. daisyUI 클래스만으로 구성하면 나중에 테마를 바꾸는 것으로 외형을 조정할 수 있다.

**정적 자원은 빌드 산출물을 jar에 포함한다.** CDN을 쓰면 아래 2.5의 콘텐츠 보안 정책에 외부 도메인을 열어야 하고, 외부 장애가 화면 장애가 된다.
...
| HTTPS 강제 | Cloudflare Tunnel 이 종단이므로 HSTS 헤더 부여 |
| 세션 | 로그인 시 세션 ID 교체(고정 공격 방지), 유휴 타임아웃, 동시 세션 1개 |
| 인증정보 노출 | 채널 API 키는 **화면에 값을 표시하지 않는다.** 등록 여부와 키 이름만 보여준다 |
| 오류 응답 | 예상하지 못한 예외는 내부 메시지를 노출하지 않는다(4.8 참조) |

> **설계 근거**: 인증정보 마스킹이 특히 중요하다. AES-GCM으로 암호화해 저장하는 이유가 유출 방지인데, 화면에서 평문을 되돌려 보여주면 그 노력이 무의미해진다. 갱신은 새 값을 입력받는 방식으로만 지원한다.
...
> **주의 — record 컴포넌트와 정적 팩토리 이름이 겹치면 컴파일이 실패한다.** record는 컴포넌트와 같은 이름의 메서드를 accessor로 취급한다. 컴포넌트 `boolean success`가 있는데 `success()` 정적 팩토리를 만들면 반환 타입이 `boolean`이어야 한다는 오류가 난다. `of()`·`from()`·`sent()` 처럼 겹치지 않는 이름을 쓴다.

### 4.8 예외 — 하나의 예외, 코드로 구분

예외 클래스를 실패 유형마다 만들지 않는다. **`BusinessException` 하나**만 두고 구분은 `ErrorCode`가 담당한다.
...
| 구성 | 책임 |
|---|---|
| `BusinessException` | 유일한 비즈니스 예외. `ErrorCode`·메시지 인자·`details` 보유 |
| `ErrorCode` | 코드 문자열 · HTTP 상태 · 로그 레벨을 한 곳에 모은 enum |
| `errors.properties` / `errors_en.properties` | 메시지 외부화 (한글/영문) |
| `ErrorMessageResolver` | 로케일에 맞는 문구 조립 |
| `GlobalExceptionHandler` | `@RestControllerAdvice` 전역 처리 |

> **설계 근거**: 예외 클래스를 나누면 새 실패 유형마다 클래스가 늘고, HTTP 상태를 화면 계층이 매번 판단해야 하며, 문구가 코드에 박혀 다국어가 불가능해진다.

#### ErrorCode를 단일 enum으로 둔다

도메인별 인터페이스 구현체로 나누는 방식도 검토했으나 **확장 비용이 더 컸다.**
...
> **설계 근거**: 실제로 구현체를 분리한 상태에서 `SKU_ID_REQUIRED`가 두 enum에 중복 정의됐고, 파일이 나뉘어 있어 작성 중에는 드러나지 않았다. 또한 검증 테스트에 클래스를 손으로 등록하는 구조라 빠뜨리면 검증 자체가 무력화된다. "공통 모듈이 도메인 상수를 안다"는 우려는 실체가 없다 — 상수 이름이 있을 뿐 의존 방향은 변하지 않는다.

접두사로 도메인을 구분한다 — 공통 `C`, SKU `S`, 주문 `O`, 매핑 `M`, 채널 `H`, 알림 `N`, 인증 `A`.

#### 로그 레벨을 ErrorCode가 정한다

전역 핸들러가 예외마다 조건 분기로 레벨을 정하면 새 코드가 추가될 때마다 핸들러를 수정해야 한다.

> **설계 근거**: 대표 사례가 `ORDER_LINE_DUPLICATE`다. 중복 수집은 **오류가 아니라 정상 흐름 제어**이므로 경고로 남으면 안 되는데(수집 커서가 구간을 겹쳐 조회하므로 반복 수집이 정상), enum에 `DEBUG`만 적으면 핸들러는 그대로 둘 수 있다.
...
#### 메시지 로케일 — `fallback-to-system-locale`은 반드시 false

MessageSource는 `요청 로케일 → 시스템 로케일 → 기본 번들` 순으로 찾는다. 이 값이 기본값 `true`면 `ko` 요청에 `errors_ko.properties`가 없을 때 시스템 로케일로 넘어가므로, **서버 OS 로케일이 사용자에게 나가는 언어를 결정한다.**

> **설계 근거**: 다국어를 지원하도록 만들어놓고도 배포 환경의 OS 설정에 좌우되는 상태가 된다. NAS 컨테이너 로케일이 무엇이냐에 따라 한국어를 요청했는데 영문이 나갈 수 있다.
...
**재고 변경과 이력 기록은 서비스가 함께 수행한다.** 호출자가 `StockMovement`를 직접 만들지 않는다 — 둘을 각각 호출하는 구조로 두면 언젠가 한쪽을 빠뜨리고, 그 순간 5.3의 대조 배치가 불일치를 보고한다.

**변경 메서드는 `Propagation.MANDATORY`다.** 3.4가 정한 "도메인 서비스는 자체 트랜잭션을 열지 않는다"를 런타임에 강제하기 위함이다.

> **설계 근거**: 애너테이션을 그냥 생략하면 호출자가 트랜잭션을 잊었을 때 재고 변경과 이력 기록이 **각각 커밋**되어, 중간 실패 시 정합성이 깨진 채 조용히 넘어간다. MANDATORY는 그 실수를 즉시 예외로 드러낸다. 컴파일 단계에서 막을 수는 없지만 첫 실행에서 반드시 드러나며, 조용히 잘못 동작하는 것보다 낫다.

### 5.2.1 오버셀링 — 음수 재고를 허용한다

기능 명세서에 규정되지 않은 영역이라 여기서 정한다.

채널 재고 동기화(F-08)는 즉시 반영이 아니라 수 분 내 반영이므로, 그 사이 **실재고를 초과하는 주문이 접수될 수 있다.**

| 선택지 | 결과 |
|---|---|
| 주문 저장을 실패시킨다 | **데이터 유실.** 고객은 이미 결제했고 채널에는 주문이 존재한다 |
| 주문만 저장하고 재고를 반영하지 않는다 | `stock_applied = false`로 남아 **소급 반영 대상으로 무한 재시도**된다 |
| **음수 재고를 허용한다** | 사실을 정확히 표현하고 대조 배치가 계속 성립한다 |

> **설계 근거**: 재고 시스템에서 음수는 "출고해야 할 빚"을 표현하는 정상적인 상태다. 화면과 알림으로 발주자가 즉시 인지할 수 있고, 이력 누적합과 스냅샷이 여전히 일치하므로 5.3의 대조 검증이 거짓 경고를 내지 않는다. 반대로 음수를 막으면 위 두 선택지 중 하나를 감수해야 하는데 둘 다 더 나쁘다.

**판매 차감만 음수를 허용한다.** 폐기·분실 같은 수동 조작은 사람의 입력 실수일 가능성이 높으므로 계속 거부한다.

```java
void decrease(int amount);            // 수동 — 음수 거부
boolean decreaseForSale(int amount);  // 판매 — 음수 허용, 오버셀링 여부 반환
```

음수 도달 시 `OVERSELL` 알림을 발송한다. 출고 불가 상태이므로 발주자가 즉시 조치해야 한다.
...
이 요건의 본질은 **알림 대상을 데이터베이스에 먼저 남긴다**는 것이며, Outbox 패턴이 그것을 구현한다.

```
주문 저장 + outbox 행 삽입        ← 동일 트랜잭션 (원자적)
        │ 커밋
        ▼
relay(스케줄러) → 텔레그램 발송 → outbox 상태 SENT
```

| 장애 지점 | 결과 |
|---|---|
| 텔레그램 API 실패 | 상태가 SENT로 전이되지 않아 재시도 대상으로 유지 |
| 애플리케이션 재기동 | 기동 시 미발송 Outbox 행을 이어서 발송 (**A-14 충족**) |
| 발송 대상 미설정 | `PENDING` 유지. 텔레그램 연결 후 일괄 발송 |

### 7.1 메시지 큐를 도입하지 않는다 — 초기 설계 변경

**초기 설계는 `Outbox → relay → RabbitMQ → 워커` 구조였으나 구현 단계에서 철회했다.**

| RabbitMQ가 주는 것 | 현 상황 |
|---|---|
| 발송 워커 프로세스 분리 | 단일 배포(NAS 1대, JVM 1개)라 무의미 |
| 부하 분산 | 일 주문 수십 건이라 무의미 |
| 재시도·실패 격리 | **Outbox가 `attemptCount`로 이미 관리한다** |

> **설계 근거**: 결정적인 문제는 **재시도 근거가 두 곳으로 갈린다**는 점이다. relay가 큐에 발행한 뒤 Outbox를 `PENDING`으로 두면 같은 알림을 반복 발행하고, `SENT`로 바꾸면 발송 실패를 Outbox가 알지 못한다. 어느 쪽이 진실인지 알 수 없는 상태가 되므로 **재시도의 단일 근거를 Outbox로 고정**한다.

미사용 의존성(`spring-boot-starter-amqp`)도 제거한다. 컨텍스트에 불필요한 자동 구성을 붙이기 때문이다. `docker-compose.yml`의 RabbitMQ 컨테이너는 남기되 현재 미사용임을 명시한다.

**인수조건 A-14는 그대로 충족된다.** 유실 방지의 실제 근거가 Outbox이며, relay가 기동 시 1회 실행되어 재기동 전에 남은 알림을 이어서 발송한다.

발송 워커를 별도 프로세스로 분리하거나 처리량이 늘어나면 큐를 다시 도입한다. 그 시점에 `palim-notification` 내부에 발송 인터페이스를 추출하면 되며, 기계적인 리팩터링이다. **구현체가 하나뿐인 인터페이스를 미리 만들지 않는다.**

### 7.2 발송 실패는 예외가 아니라 결과로 표현한다

```java
public record TelegramSendResult(boolean success, boolean retryable, String errorMessage)
```

네트워크는 끊기고 외부 API는 느려진다. 그때마다 예외를 던져 스택 트레이스를 남기는 것은 과하다. 더 중요한 것은 **`retryable`을 호출자가 반드시 확인하게 만드는 것**이다 — 예외로 표현하면 이 구분이 예외 타입이나 메시지 문자열에 묻힌다.

| 응답 | 처리 |
|---|---|
| 4xx | **즉시 `FAILED`.** 잘못된 chat_id·봇 차단·토큰 무효는 재시도해도 성공하지 않고 호출 제한만 소모한다 |
| 5xx·타임아웃 | `attemptCount` 증가. 한도 초과 시 `FAILED` |

### 7.3 긴급 알림은 야간 보류·묶음에서 제외한다

`OVERSELL`, `OUT_OF_STOCK`, `COLLECT_FAILURE`, `STOCK_PUSH_FAILURE`, `STOCK_MISMATCH`는 발송 방식 설정과 무관하게 즉시 발송한다.

> **설계 근거**: 재고가 음수가 되었거나 수집이 멈춘 상황은 아침까지 기다릴 수 없다. 발주자가 알림 과다를 우려해 야간 보류를 켰더라도, 매출 손실로 직결되는 사안은 즉시 알려야 한다.

묶음 발송 대상은 주문 알림뿐이다. 묶음은 주문량이 많을 때 알림 과다로 확인하지 않게 되는 문제를 막기 위한 기능이므로(F-02), 빈도가 낮은 다른 알림은 묶을 이유가 없다.
