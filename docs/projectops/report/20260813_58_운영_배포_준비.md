...
palim 을 처음으로 운영 배포했다. **`https://palim.suhsaechan.kr` 응답까지 실측 확인**했다.
...
| 헬스체크 | `{"status":"UP"}` |
| 도메인 | `https://palim.suhsaechan.kr` → `302 /login` → **`200`, 로그인 화면 렌더** |
...
NAS `/volume1/docker/palim/` 에 배포 방식을 정하기 전 만들어 둔 `.env` 와
