package kr.suhsaechan.palim.common.crypto;
 ...
 *
 * <p>암호화는 특정 도메인의 것이 아니라 <b>공통 관심사</b>다. 연동 인증정보든 채널 API 키든
 * 같은 방식으로 다뤄야 하므로 여기 둔다. 동결 도메인({@code palim-channel})에 같은 구현이
 * 남아 있지만 그쪽은 수정 대상이 아니라 그대로 둔다 — 새로 만드는 것은 전부 이쪽을 쓴다.
 *
...
public class SecretCipher {
    ...
    public SecretCipher(@Value("${palim.crypto.master-key}") String base64MasterKey) {
    ...
    private static byte[] decodeKey(String base64MasterKey) {
        if (base64MasterKey == null || base64MasterKey.isBlank()) {
            throw new IllegalStateException(
                    "마스터키가 설정되지 않았습니다. PALIM_CRYPTO_MASTER_KEY 를 지정하세요.");
        }
        try {
