...
public enum ApiAuthPreset {
    ...
    /**
     * 파일을 어디서 어떻게 받는지 — <b>실제 경로.</b>
     *
     * <p>자동 수집이 깨졌을 때 쓰는 우회로인데 그때 방법을 찾아다니게 하면 우회로가 아니다.
     * 그래서 <b>어느 메뉴에서 어떻게 받는지를 여기 적는다.</b>
     *
     * <p>「실제 경로를 확인하신 뒤 적어 두세요」 라고 쓰지 않는다 — 그것은 제품이 할 일을
     * 사람에게 미루는 것이다. 확인은 우리가 하고, 사람은 보고 따라 하기만 한다.
     *
     * <p><b>단, 상대 사이트는 언젠가 바뀐다.</b> 그래서 이 값은 «기본값» 이고 화면에서 고칠 수
     * 있으며, 화면을 읽어 올 수 있는 연동은 상대에게 직접 물어 이 자리를 갱신한다.
     */
    public String getFileGuide() {
        return switch (this) {
            case ECOUNT -> """
                    ① 이카운트에 로그인합니다
                    ② 왼쪽 메뉴에서  재고Ⅰ  →  출력물  →  재고현황  (또는 창고별재고현황)
                    ③ 기준일을 «자동 수집과 같은 날» 로 두고 [조회] 를 누릅니다
                    ④ [조회] 단추에 마우스를 올리면 나오는  Excel  을 누릅니다

                    받아 온 표에 아래 값이 있어야 합니다
                      · 품목코드 · 품목명 · 규격 · 창고 · 재고수량
                    (자동 수집은 PROD_CD · PROD_DES · PROD_SIZE_DES · WH_CD · WH_DES · BAL_QTY
                     를 씁니다. 엑셀 열 이름이 달라도 괜찮습니다 — 칸 맞추기에서 이어 줍니다)

                    ※ 기준일을 다르게 잡으면 수치가 어긋납니다. 차이가 아니라 «다른 날» 을
                       보게 되므로, 대조가 없는 차이를 있다고 말합니다.""";
            case ONEWMS -> """
                    ① ONEWMS 에 로그인합니다 (수집이 쓰는 계정과 같은 계정)
                    ② 재고 현황 화면으로 갑니다 — 화면코드 I100
                    ③ 조회 조건을 «창고 1번 · 재고구분 정상» 으로 두고 조회합니다
                    ④ 표 오른쪽 위  엑셀  아이콘을 누릅니다

                    받아 온 표에 아래 값이 있어야 합니다
                      · 상품ID · 상품명 · 정상재고
                    (표에는 바코드·공급사·옵션·로케이션·입고대기·이동준비·할당재고도 함께 나옵니다)

                    ※ 조회 조건이 다르면 수치가 어긋납니다. 차이가 아니라 «다른 조건» 을
                       보게 되므로, 대조가 없는 차이를 있다고 말합니다.
                    ※ 메뉴 «이름» 은 계정 권한에 따라 다를 수 있습니다. 아래 「메뉴 경로 확인」
                       을 누르면 그 계정에 실제로 보이는 이름을 읽어 와 이 자리에 채웁니다.""";
            case CUSTOM_FORM -> "";
        };
    }
