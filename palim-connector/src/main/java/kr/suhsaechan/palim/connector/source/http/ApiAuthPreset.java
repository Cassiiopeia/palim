...
public enum ApiAuthPreset {
    ...
    public String getFileGuide() {
        return switch (this) {
            case ECOUNT -> """
                    자동 수집과 «같은 값» 이 담긴 표를 받아야 합니다.

                    받아 온 표에 아래 값이 있어야 합니다
                      · 품목코드 · 품목명 · 재고수량

                    ※ 메뉴 이름은 이카운트 버전에 따라 다릅니다. 실제 경로를 확인하신 뒤
                       이 안내를 고쳐 두시면 다음부터 그대로 씁니다.""";
            case ONEWMS -> """
                    자동 수집이 보는 화면은 재고 현황(화면코드 I100) 이고,
                    조회 조건은 «창고 1번 · 재고구분 0(정상)» 입니다.
                    같은 조건으로 조회한 표를 받아야 수치가 맞습니다.

                    받아 온 표에 아래 값이 있어야 합니다
                      · 상품ID · 상품명 · 정상재고
                    (표에는 바코드·공급사·옵션·로케이션·입고대기·이동준비·할당재고도 함께 나옵니다)

                    ※ 메뉴 이름은 계정 권한에 따라 다릅니다. 실제 경로를 확인하신 뒤
                       이 안내를 고쳐 두시면 다음부터 그대로 씁니다.""";
            case CUSTOM_FORM -> "";
        };
    }
