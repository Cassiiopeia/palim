...
public interface UnitConversionRepository extends JpaRepository<UnitConversion, UUID> {
     ...
     *
     * <p>{@link UnitConversion#GLOBAL} 이 전역 규칙이다. 정렬로 품목별을 앞에 두어 우선순위를 한
     * 쿼리로 표현한다. 결과를 {@code List} 로 받는 이유는 두 규칙이 모두 존재하는 것이 정상이며
     * {@code Optional} 로 받으면 그때 예외가 나기 때문이다 — 첫 행만 취한다.
     *
     * <p>직접 부르지 말고 {@link #findRule} 을 쓴다. 그쪽이 {@code itemRef} 를 정규화한다.
     */
    @Query("""
            select c.factor from UnitConversion c
            where c.tenantId = :tenantId
              and (c.itemRef = :itemRef or c.itemRef = :global)
              and c.fromUnit = :fromUnit
              and c.toUnit = :toUnit
            order by case when c.itemRef = :global then 1 else 0 end
            """)
    ...
    default Optional<BigDecimal> findRule(UUID tenantId, String itemRef, String fromUnit,
                                          String toUnit) {
        return findFactors(tenantId, UnitConversion.normalizeItemRef(itemRef),
                UnitConversion.GLOBAL, fromUnit, toUnit).stream().findFirst();
    }
     ...
     *
     * <p>직접 부르지 말고 {@link #existsRule} 을 쓴다.
     */
    @Query("""
            select count(c) > 0 from UnitConversion c
            where c.tenantId = :tenantId
              and c.itemRef = :itemRef
              and c.fromUnit = :fromUnit
              and c.toUnit = :toUnit
            """)
    boolean existsRuleExact(@Param("tenantId") UUID tenantId,
                            @Param("itemRef") String itemRef,
                            @Param("fromUnit") String fromUnit,
                            @Param("toUnit") String toUnit);

    default boolean existsRule(UUID tenantId, String itemRef, String fromUnit, String toUnit) {
        return existsRuleExact(tenantId, UnitConversion.normalizeItemRef(itemRef), fromUnit, toUnit);
    }
}
