 ...
 *
 * <p>둘을 묶어 두면 <b>빠뜨릴 수 없다.</b> 새 조회를 만들 때 원천을 넘기는 순간 조건도 함께
 * 넘어간다 — 이것이 이 타입의 존재 이유다. 조건이 창고에서 「어느 칸이든」 으로 넓어져도 이
 * 자리는 그대로다.
 *
 * <p><b>조건만 담지 않는다.</b> 「어느 칸을 더할지」({@code compareField})도 같은 성격이다 —
 ...
 * @param leftFilter   좌측에서 볼 조건. 비어 있으면 전부
 * @param rightFilter  우측에서 볼 조건. 비어 있으면 전부
...
public record Pairing(String leftSource, String rightSource,
    ...
    public Pairing(String leftSource, String rightSource,
                   FilterSpec leftFilter, FilterSpec rightFilter) {
        this(leftSource, rightSource, leftFilter, rightFilter, null);
    }

    /**
     * 조건을 가리지 않는 짝.
     *
     * <p>정의가 아직 없는 자리(설정 안내, 시험)에서만 쓴다. 대조 화면이 이것을 쓰면 조건을 건
     * 뜻이 사라진다.
     *
     * <p><b>{@code of(ReconcileDefinition)} 은 두지 않는다.</b> 조건을 읽으려면 저장소가
     * 필요하고, 그것은 {@code FilterService.pairingOf} 의 일이다. 정적 팩토리를 남겨 두면
     * 조건 없는 {@code Pairing} 이 조용히 만들어져 「걸었는데 안 걸린다」 가 된다.
     */
    ...
    /**
     * 한쪽에 값이 여럿인데 <b>아무 조건도 안 건</b> 상태인가.
     *
     * <p>그대로 두면 맡기지 않은 물량까지 합산되어 조용히 틀린 답이 나온다. 화면이 이때
     * 「조건을 거세요」 를 띄운다.
     *
     * @param leftValueCount  좌측 원천에 담긴 창고 수
     * @param rightValueCount 우측 원천에 담긴 창고 수
     */
    public boolean needsChoice(int leftValueCount, int rightValueCount) {
        return (leftValueCount > 1 && leftFilter.isAll())
                || (rightValueCount > 1 && rightFilter.isAll());
    }
}
